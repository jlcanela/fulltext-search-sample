...
object Source {
  ...
  def execChunk(chunk: Chunk[ES_Search]): ZIO[Has[LogService.LogService] with console.Console, Throwable, Chunk[List[Log]]] = 
    for {
      _ <- console.putStrLn(chunk.toString)
  ...
  lazy val BatchedSearchDataSource: DataSource[Has[LogService.LogService] with console.Console, ES_Search] =
